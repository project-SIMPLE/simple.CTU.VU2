...
public class Thuan_23127_JsonReader : MonoBehaviour
{
    ...
    public Plant GetPlantById(int id)
        => GetCurrentLangPlants()?.FirstOrDefault(p => p.id == id);

    public Animal GetLivestockById(int id)
        => GetCurrentLangAnimals()?.FirstOrDefault(a => a.id == id);

    public Fish GetFishById(int id)
        => GetCurrentLangFish()?.FirstOrDefault(f => f.id == id);
